...
public class UserLoginSuccessHandler extends HandlerInterceptorAdapter {
	...
	public void postHandle(HttpServletRequest request, HttpServletResponse response, Object handler,
		...
		String encPassword = (String) mv.get("encPassword");// 컨트롤러에서 받아온 암호화된 비밀번호
		String rawPassword = (String) mv.getAttribute("rawPassword"); // 컨트롤러에서 받아온 사용자가 입력한 암호화되지 않은 비밀번호
		UserVO vo = (UserVO) mv.get("user"); // 컨트롤러에서 받아온 vo
		Boolean match = passwordEncoder.matches(rawPassword, encPassword); // 비밀번호가 일치하면 true, 불일치시 false 반환
